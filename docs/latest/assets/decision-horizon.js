...
function play(){cancelAnimationFrame(raf);clearTimeout(settleTimer);scene.classList.remove('ready','settled');void scene.offsetWidth;start=performance.now();scene.classList.add('ready');if(!reduce)raf=requestAnimationFrame(draw);settleTimer=setTimeout(()=>scene.classList.add('settled'),5500)}
