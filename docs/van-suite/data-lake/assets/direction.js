(()=>{
  ...
  function set(value){
    ...
    root.dataset.mode=v>=95?'back':v<=5?'forward':'split';
    range.value=v;
    buttons.forEach(button=>button.classList.toggle('active',Number(button.dataset.value)===v));
    range.setAttribute('aria-valuetext',v===100?'Classic architecture: look back':v===0?'Cerniam architecture: look forward':`${100-v} percent Cerniam architecture revealed`);
  }
